"""
Train models with Advanced Features V3 (CLEAN - NO LEAKAGE)
Goal: Achieve <45% SMAPE (from 47.52% baseline)

REMOVED ALL TARGET LEAKAGE:
- estimated_unit_price (used target price)
- brand_avg_price, brand_median_price, brand_price_std (used target prices)
- category_avg_price, category_median_price (used target prices)
"""
...
print("\n" + "="*70)
print("TRAINING WITH ADVANCED FEATURES V3 (CLEAN - NO LEAKAGE)")
print("="*70)
...
# ==================================================
# Strategy 1: Single Train/Val Split
# ==================================================
print("\n" + "="*70)
print("STRATEGY 1: SINGLE TRAIN/VAL SPLIT")
print("="*70)
...
lgb_smape = smape(y_val, lgb_pred_val)
...
xgb_smape = smape(y_val, xgb_pred_val)
...
cat_smape = smape(y_val, cat_pred_val)

print(f"\n✓ CatBoost Val SMAPE: {cat_smape:.4f}%")
...
results = pd.DataFrame({
    ...
    'Val SMAPE': [lgb_smape, xgb_smape, cat_smape]
}).sort_values('Val SMAPE')

print("\n", results.to_string(index=False))

best_model_name = results.iloc[0]['Model']
best_smape = results.iloc[0]['Val SMAPE']

print(f"\n🏆 Best Model: {best_model_name}")
print(f"📊 Best Val SMAPE: {best_smape:.4f}%")
...
print(f"New SMAPE: {best_smape:.2f}%")
print(f"Improvement: {improvement:.2f}% ({improvement_pct:.1f}% reduction)")

if best_smape < 45:
    print(f"\n🎉 SUCCESS! Achieved <45% SMAPE goal!")
elif best_smape < baseline_smape:
    print(f"\n✅ PROGRESS! Improved from baseline")
else:
    print(f"\n⚠️  No improvement yet. Need more features.")
...
output_path = model_dir / 'test_out_v3_clean.csv'
...
feature_importance.to_csv(model_dir / 'feature_importance_v3_clean.csv', index=False)
print(f"\n✓ Feature importance saved: feature_importance_v3_clean.csv")

print("\n" + "="*70)
print("✓ TRAINING COMPLETED (CLEAN - NO LEAKAGE!)")
print("="*70)
print(f"\n🏆 Best Model: {best_model_name}")
print(f"📊 Val SMAPE: {best_smape:.4f}%")
