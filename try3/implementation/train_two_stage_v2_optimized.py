"""
Two-Stage DistilBERT V2: OPTIMIZED VERSION

IMPROVEMENTS FROM V1:
1. Simplified to 3 price ranges (was 5) - easier to classify
2. Higher classification weight (0.5 vs 0.3) - better range prediction
3. Lower Focal Loss gamma (1.0 vs 2.0) - less aggressive
4. Unified Huber Loss for all ranges (was mixed) - more consistent
5. Better class balance handling

Expected: 41-42% SMAPE (vs 54% from V1)

CHANGELOG:
- V1 Results: Fold 1: 54.2%, Fold 2: ~56% (WORSE than 47.4% baseline)
- V2 Fixes: Simpler architecture, better loss balance
"""
...
print("="*80)
print("🚀 Two-Stage Model V2: OPTIMIZED - Classification + Regression")
print("="*80)
print()
...
CONFIG = {
    # Paths
    'data_dir': DATA_DIR,
    'output_dir': OUTPUT_DIR / 'two_stage_v2_optimized',
    
    # Model
    'model_name': 'distilbert-base-uncased',
    'max_length': 128,
    
    # SIMPLIFIED: 3 price ranges (was 5)
    'price_ranges': [
        (0, 20, 'Budget_Economy'),     # Merge Budget + Economy (64% of samples)
        (20, 100, 'Mid_Premium'),      # Merge Mid + High-End (33% of samples)
        (100, np.inf, 'Luxury'),       # Keep Luxury separate (3% of samples)
    ],
    ...
    # IMPROVED: Loss weights (classification more important)
    'classification_weight': 0.5,  # Was 0.3
    'regression_weight': 0.5,      # Was 0.7
...
print(f"📍 Price Ranges: {len(CONFIG['price_ranges'])} (SIMPLIFIED from 5)")
print(f"📍 Loss Balance: Class={CONFIG['classification_weight']}, Reg={CONFIG['regression_weight']}")
...
# ============================================================================
# Focal Loss (LESS AGGRESSIVE)
# ============================================================================

class FocalLoss(nn.Module):
    """Focal Loss with reduced gamma"""
    def __init__(self, alpha=None, gamma=1.0):  # Was 2.0
        super().__init__()
        self.alpha = alpha
        self.gamma = gamma
...
# ============================================================================
# UNIFIED: Huber Loss for All Ranges
# ============================================================================

class HuberLoss(nn.Module):
    """Huber loss - works well for all price ranges"""
...
class TwoStageModel(nn.Module):
    """Simplified Two-Stage Model with 3 ranges"""
    ...
    def __init__(self, model_name, n_ranges):
        ...
        # SIMPLIFIED: Smaller classification head
        self.classifier = nn.Sequential(
            nn.Linear(768, 128),  # Was 256
            nn.ReLU(),
            nn.Dropout(0.2),
            nn.Linear(128, n_ranges)
        )
        
        # Range-specific regression heads
...
def train_epoch(model, dataloader, optimizer, scheduler, class_criterion, 
                reg_criterion, device, fp16=False):
    """Train for one epoch"""
    model.train()
    total_loss = 0
    total_class_loss = 0
    ...
    for batch in pbar:
        ...
        if fp16:
            with torch.cuda.amp.autocast():
                ...
                reg_loss = reg_criterion(reg_preds, price_log)
...
def main():
    ...
    print("\n📊 Price Range Distribution (SIMPLIFIED):")
    for i, (low, high, name) in enumerate(CONFIG['price_ranges']):
    ...
    # Loss functions
    class_criterion = FocalLoss(alpha=class_weights, gamma=1.0)  # REDUCED from 2.0
    reg_criterion = HuberLoss(delta=1.0)  # UNIFIED for all ranges
    ...
    print("\n🚀 Starting cross-validation (V2 OPTIMIZED)...")
    print("="*80)
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(train_df, train_df['price_range']), 1):
        ...
        if fold_model_path.exists():
            print(f"\n✅ Fold {fold}/{CONFIG['n_folds']} - Already trained! Loading...")
            print("-"*80)
            ...
            fold_smape, fold_class_acc, fold_preds, _ = evaluate(
                model, val_loader, class_criterion, reg_criterion, CONFIG['device']
            )
        ...
        for epoch in range(CONFIG['num_epochs']):
            ...
            train_loss, train_class_loss, train_reg_loss = train_epoch(
                ...
                class_criterion, reg_criterion, CONFIG['device'], CONFIG['fp16']
            )
    ...
    print("\n" + "="*80)
    print("📊 CROSS-VALIDATION RESULTS (V2 OPTIMIZED)")
    print("="*80)
    ...
    print(f"Two-Stage V1:        54.237% (WORSE ❌)")
    print(f"Two-Stage V2:        {oof_smape:.3f}%", end="")
    if oof_smape < 47.378:
